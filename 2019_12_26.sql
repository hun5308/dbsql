...
--여러건의 SELECT 결과 존재하는 상황에서 스칼라 변수에 값을 넣는 상황
...
--해당 예외를 잡아 NO_EMP라는 개발자가 정의한 예외로 재정의하여 예외를 던질 수 있다.
...
SELECT getdeptname(deptno), --4가지
        getdeptname(empno) --row 수만큼 데이터가 존재
FROM emp; 
...
--users 테이블의 비밀번호 컬럼이 변경이 생겼을 떄
--기존에 사용하던 비밀번호 컬럼 이렬을 관리하기 위한 테이블
...
--timing
    ...
    FOR EACH ROW  --행 트리거, 행의 변경이 있을 떄마다 실행한다
    --현재 데이터 참조 : OLD
    --갱신 데이터 참고 : NEW
    BEGIN
        --users 테이블의 pass컬럼을 변경할 때 trigger 실행
...
--trigger에 의해서 users_hustory 테이블에 이력이 생성되는지 확인
SELECT *
FROM USERS_HISTORY;
